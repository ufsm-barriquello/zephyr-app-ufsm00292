...
static int send_udp_data(struct sample_data *data)
{
    ...
    float temperatura = get_sensor_temp(); 

    len = snprintf(data->udp.last_tx_buf, MAX_SENSOR_STRING_LEN,
                   "Temperatura: %.1f C", (double)temperatura);
    if (len < 0 || len >= MAX_SENSOR_STRING_LEN) {
        LOG_ERR("Falha ao formatar a string do sensor");
        return -EIO;
    }

    data->udp.expecting = len + 1;

    ret = send(data->udp.sock, data->udp.last_tx_buf, data->udp.expecting, 0);

    LOG_INF("A enviar: '%s' (%d bytes)",
            data->udp.last_tx_buf, data->udp.expecting);
